...
def build_decision_prompt(
    ...
    if "[EditArtifact]" in community_summary and "REVIEW CHANGES" in community_summary:
        artifact_urgency += (
            "\n🗣️ **DEBATE THE EDITS — comment on EditArtifact proposals you see.**\n"
            "An EditArtifact proposal is a creative / substantive change to the work. The CURRENT vs\n"
            "PROPOSED diff is shown above in the proposal listing — **READ IT**. Then leave an honest\n"
            "editor-in-a-writers'-room comment.\n"
            "\n"
            "🚫 **HARD RULE — NO HALLUCINATING.** Your comment MUST quote a literal substring (5+ words,\n"
            "in double quotes) copied character-for-character from the **PROPOSED** text shown in this\n"
            "prompt. Do NOT invent artifact titles, character names, plot beats, or themes that are\n"
